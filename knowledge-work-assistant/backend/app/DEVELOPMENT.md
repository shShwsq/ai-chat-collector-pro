...
├── main.py              # FastAPI 入口：lifespan + CORS + 13 个 router 挂载 + /ws
...
├── routers/             # 13 个 FastAPI 路由模块（详见 routers/DEVELOPMENT.md）
└── services/            # 16 个业务 service 模块（详见 services/DEVELOPMENT.md）
```
...
| `main.py` | FastAPI 入口 | `lifespan`（加载 `_REGISTRY.load()` → `await init_db()` → `await migrate_node_columns(engine)` → `init_graph_agent()`）；CORS 允许 `["http://localhost:5174","file://"]`；按 `/api` 前缀挂载 13 个 router + `/ws`；`if __name__ == "__main__"` 提供 `uv run python -m app.main` 便捷入口（生产推荐用 `uvicorn` 命令） |
...
`main.py` 用 `app.include_router(..., prefix="/api", tags=[...])` 挂载所有业务路由，前缀统一 `/api`；`ws.py` 挂载在根路径 `/ws`。13 个 router 按业务域拆分（详见 [routers/DEVELOPMENT.md](./routers/DEVELOPMENT.md)）：
...
`init_db` → `migrate_node_columns` → `init_graph_agent` 的顺序基于依赖关系：
- `migrate_node_columns` 用 `PRAGMA table_info(nodes)` 检查列，必须在 `init_db` 创建 `nodes` 表之后。
- `init_graph_agent` 内部可能调 `llm_factory.get_llm_client`，依赖 `model_config._REGISTRY` 已加载。
顺序乱会导致启动期异常或运行时 `graph_agent` 用空注册表。
