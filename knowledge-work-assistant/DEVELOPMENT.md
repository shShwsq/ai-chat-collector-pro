...
| `backend/app/services/` | 服务层 | 21+ 个 service 模块 + `tools/` / `multimodal/` / `prompts/` 子包；`graph_store`（图谱 CRUD + JSON 透明序列化）+ `graph_agent`（图谱 AI Agent，封装 LLM 调用 + 流式 + 降级）+ `main_agent` / `writer_agent` / `sub_agent`（多 Agent 协作）+ `llm_client`/`llm_factory`/`llm_errors`/`llm_request_registry`/`model_config` + `crypto`/`settings_store` + `ws_notify`/`session_queue` + `tag_store`/`knowledge_store`/`file_storage` |
