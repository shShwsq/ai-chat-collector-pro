...
- **`helpers/load-source.js`**：唯一的 ES module 文件（`import fs from 'node:fs'`）。提供 `runInWindow(filePath)`（读源文件 → `const`/`let` 转 `var` → indirect eval 在 jsdom 全局执行）、`mockChrome(overrides)`（挂 `window.chrome.runtime` / `chrome.storage.local` mock）、`setBody(html)` / `setDocument(html)` / `setPathname(pathname)` / `setTitle(title)`（jsdom 环境辅助）。导出 7 个 loader 函数：`loadDb()` / `loadEmbedding()` / `loadVectorStore()` / `loadLlm()` / `loadHtmlToMarkdown()` / `loadDomAdapter(platform)` / `loadSanitizeHtml()`，每个 loader 先 `mockChrome()` 再 `runInWindow` 加载源文件及其依赖，返回需要测试的全局对象。

### DOM 适配器测试（4 个，"平台 DOM 改了立刻发现"防线）
...
- **`dom/markdown-safety.test.js`**：HTML 净化器 `HtmlSanitizer` 测试。覆盖标签白名单过滤（危险标签如 script/style/iframe/svg/math 被移除）、属性白名单过滤（on* 事件属性、非法 href 被剥离）、协议白名单校验（javascript: 协议被替换为 #）、强制外链安全属性（`target="_blank"` + `rel="noopener noreferrer"`）、非 HTML namespace 元素移除、mark 元素保留等场景。
...
- **`unit/llm.test.js`**：`lib/llm.js` 纯函数测试。覆盖 `_buildOpenAIChatUrl`（baseUrl 智能拼接 `/chat/completions`）、`_buildThinkingExtras`（**项目最复杂的函数**：6 厂商 × 3 思考模式 × 2 开关 = 36 种组合）、`AIAssistant._parseEmbId`（embId 格式解析）、`AIAssistant._buildContexts`（RAG 上下文构建：embId 映射校验、缺失 convId 跳过、无法映射跳过）。`_buildThinkingExtras` 测试用例覆盖 project_memory 中记录的全部厂商差异：DashScope/Qwen 用 `enable_thinking` 布尔值；DeepSeek/智谱/Kimi 用 `thinking` 对象 `{type:"enabled"/"disabled"}`；豆包用 `thinking` 对象 + `fallbackThinking`（Endpoint ID 匹配不上 modelMeta）；MiniMax 用 `thinking {type:"adaptive"/"disabled"}` + `reasoning_split:true`。
...
| `dom/markdown-safety.test.js` | HTML 净化器测试 | `loadSanitizeHtml()` 加载 `HtmlSanitizer`；`describe('标签白名单')`（危险标签移除/安全标签保留）；`describe('属性白名单')`（on* 属性剥离/class 保留）；`describe('协议安全')`（javascript: → # / http/https 保留）；`describe('外链安全')`（自动添加 target="_blank" + rel="noopener noreferrer"）；`describe('非 HTML 元素')`（SVG/MathML 移除）；`describe('mark 元素')`（高亮标签保留） |
