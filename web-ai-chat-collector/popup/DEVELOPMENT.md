...
6. **`settings.js`**：设置页逻辑（约 1300 行，项目最大的 JS 文件）。`DOMContentLoaded` 后：`loadModelsCatalog()` 拉取 `models.json` → 填充下拉 → `loadSettings()` 读用户配置填表单 → 记录 `formSnapshot`（用于未保存提示）。核心函数：`saveSettings()`（按 6 类顺序保存：platforms → platformModes → embedding → retrieval → vectorStore → llm，**安全加固**：保存前先进行远程端点 URL 校验 + 非 HTTPS 警告 + host 权限申请，用户拒绝则不保存，避免留下半配置状态；含切片校验、向量库后端切换询问）、`saveVectorStoreSettings({ interactive })`（interactive=true 弹后端切换询问，false 静默保存给测试连通性用）、`testEmbedding()` / `testLLM()` / `testVectorConnection()`（三者都是先保存再测试）、`rebuildIndex()`、`loadStorageInfo()`、`loadVectorStoreStats()`、`clearConversations()` / `resetSettings()`、自定义模型下拉交互（`renderModelOptions` / `openModelDropdown` / `closeModelDropdown`，含键盘导航 Enter/Escape）、`applyOpenaiPreset(providerId)`（预设厂商切换时自动填 baseUrl/模型列表/思考开关）、`updateOpenaiThinkingByModel()`（根据模型 thinking 模式更新开关状态）、`updateRetrievalModeUI()`（按 mode 显示/隐藏 TopK 与阈值输入框）、`openHelp(type)`（动态加载 `docs/<xxx>-setup.md` 渲染到帮助弹窗）、`urlToOrigin(rawUrl)`（提取 origin 用于权限申请）、`validateRemoteEndpoint(rawUrl)`（**新增**：校验远程端点 URL 格式、协议、是否为本地、是否为非 HTTPS 远程）、`ensureHostPermission(rawUrl)`（申请远程域名 host 权限，保存前申请而非保存后）、`showToast(message, isError)`、`toggleCollapse(headerEl, bodyEl)`。
...
| `popup.js` | 弹窗逻辑 | `loadStatus()`（发 `GET_STATUS`，更新状态栏 + 平台过滤选项）、`loadConversations()`（按 `currentSearchQuery` 发 `SEARCH_CONVERSATIONS` 或 `GET_CONVERSATIONS`）、`createConvItem(conv)`（渲染卡片，含 6 条消息预览 + 4 按钮）、`openViewer(conv)`（弹查看器，渲染所有消息）、`renderViewerContent(content)`（Markdown+KaTeX+折叠块渲染，**安全加固**：marked 输出经 `HtmlSanitizer.sanitize()` 净化后再插入 DOM）、`renderMath(tex, displayMode)`（KaTeX 渲染）、`sendMessage(msg)`（Promise 包装 `chrome.runtime.sendMessage`）、`escapeHtml(text)`、`handleExportAll()`（confirm 选 markdown/json）、`handleSearch()` |
...
| `settings.js` | 设置页逻辑（约 1300 行） | `loadModelsCatalog()`（fetch models.json 填充下拉）、`loadSettings()`（发 6 次 `GET_SETTINGS` 填表单 + `formSnapshot`）、`saveSettings()`（按 6 类保存 + **保存前安全校验 + 权限申请** + 非 HTTPS 警告）、`saveVectorStoreSettings({ interactive })`（向量库专门保存，含后端切换询问）、`applyOpenaiPreset(providerId)`（预设厂商切换）、`updateOpenaiThinkingByModel()`、`updateEmbeddingModelHelp()`（显示维度/多模态/dimensionsParam 提示）、`renderModelOptions(keyword)` / `openModelDropdown()` / `closeModelDropdown()`（自定义下拉）、`renderEmbeddingOptions(keyword)` / `openEmbeddingDropdown()` / `closeEmbeddingDropdown()`、`testEmbedding()` / `testLLM()` / `testVectorConnection()` / `rebuildIndex()`、`loadStorageInfo()` / `loadVectorStoreStats()`、`clearConversations()` / `resetSettings()`、`openHelp(type)` / `closeHelp()`、`urlToOrigin(rawUrl)` / `validateRemoteEndpoint(rawUrl)`（**新增**：URL 格式/协议/本地/HTTPS 校验） / `ensureHostPermission(rawUrl)`（保存前权限申请）、`showToast(message, isError)` / `toggleCollapse(headerEl, bodyEl)`、`serializeForm()` / `isFormDirty()`（未保存提示）、`updateRetrievalModeUI()` / `updateVectorHelpLink()` |
