...
4. **`llm.js`**：LLM 服务单例 `LLMService` + AI 助手 `AIAssistant`。LLMService 支持 OpenAI 兼容（6 个预设厂商）+ Ollama 本地，统一流式接口 `chatStream(messages, onChunk, options)`，思考参数注入（`_buildThinkingExtras` 处理 6 厂商 × 3 思考模式 × 2 开关的矩阵），SSE 解析（区分 `reasoning_content` 与 `content` 两个字段）。AIAssistant 提供三种 RAG 模式：`organizeInfo` / `generateQuiz` / `askQuestion`，统一流程：embed query → retrievalSearch → _buildContexts（父子检索：命中消息+前后各 1 条邻居）→ 构造 prompt → chatStream。**安全加固**：移除不安全的降级路径，RAG 上下文构建严格校验 embId 映射，无法验证的命中直接跳过而非静默扩大为整段对话，避免无关内容污染检索结果。
5. **`sanitize-html.js`**：HTML 白名单净化器 `HtmlSanitizer`。基于 DOMParser 的 DOM 级净化，仅接受 marked 生成的 HTML。支持标签白名单（34 个常用 HTML 标签）、属性白名单（class/title/target/rel/href/colspan/rowspan）、协议白名单（http/https/mailto）。自动移除 SVG/MathML 等危险元素，强制外链 `target="_blank"` + `rel="noopener noreferrer"`。用于 popup 查看器和 content viewer 的 Markdown 输出安全渲染。
