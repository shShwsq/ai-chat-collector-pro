...
- **`conversations.js` 的 `dbSaveConversation`**：默认行为只写本地 IndexedDB；启用对接后，`bg/local-app.js` 的 `LocalApp_pushByConvId` 会在保存后即时推送（由 `bg/router.js` 在 SAVE_CONVERSATION 处理完成后异步调用），POST 到 `http://127.0.0.1:8788/api/plugin/conversations`，落库为 `Observation`。
...
- **推送链路鉴权**：KWA 后端当前不鉴权，仅适用于 loopback（`127.0.0.1:8788`）；推送 URL 在 collector 设置页"本地应用对接"分区配置，默认 `http://127.0.0.1:8788`。
...
| `local-app.js` | 本地软件（KWA）对接 | **常量**：`LOCAL_APP_DEFAULT_URL`（`http://localhost:8788`）、`LOCAL_APP_ALARM`（`local-app-push`）、`PUSHED_KEY`（`localAppPushedConvIds`）、`SETTINGS_KEY`（`localAppSettings`）、`PLATFORM_MAP`（插件平台 → 后端白名单映射）。**设置**：`getLocalAppSettings()` / `saveLocalAppSettings(settings)`（enabled/autoPush/pushOnSave/intervalMinutes/baseUrl）。**已推送映射**：`loadPushedMap()` / `savePushedMap()`（避免重复请求已推送且未变更的对话）。**初始化**：`LocalApp_init()`（加载设置 + 推送映射 + 同步 alarm + 注册 alarm 监听器）。**定时器**：`_syncAlarm()`（chrome.alarms 最小 1 分钟）、`_onAlarm()`（定时推送入口）。**推送**：`_buildRequestBody(conv)`（转换为后端 POST /api/plugin/conversations 请求体，含 metadata.conversation_id 用于 24h 幂等去重）、`LocalApp_pushConversation(conv)`（单条推送，增量优化跳过未变更对话）、`LocalApp_pushAll({silent})`（全量推送，网络错误时停止后续推送）、`LocalApp_pushByConvId(convId)`（保存后即时推送）。**连通性**：`LocalApp_testConnection()`（调用 GET /api/plugin/health）。**状态**：`LocalApp_getStatus()`（已推送统计）。**设置钩子**：`LocalApp_applySettings(newSettings)`、`LocalApp_resetPushedMap()` |
